...
class WaveformUI:
    def __init__(self, width: int = 800, height: int = 200, window_name: str = "Magic Mirror - Waveform"):
        ...
        # Idle motion parameters: keep a subtle wave when no audio
        self.idle_amplitude: float = 0.06
        self.idle_after_s: float = 0.25
    ...
    async def run(self) -> None:
        ...
        try:
            while self.is_running:
                ...
                # Idle amplitude floor for gentle motion when silent
                idle_amp = 0.0
                idle_dt = time.time() - self._last_audio_time
                if idle_dt > self.idle_after_s:
                    # Slow breathing modulation so it doesn't feel static
                    idle_amp = self.idle_amplitude * (0.6 + 0.4 * np.sin(self.wave_phase * 0.6))
                effective_amp = max(self.current_amplitude, idle_amp)
                ...
                thickness_base = int((6 + effective_amp * 6) * self.ss)
